...
| Navbar Logout | `POST /api/v1/user/patient/logout` or `POST /api/v1/user/doctor/logout` | Clears session and redirects to login | Implemented + loading state |
...
| Sidebar logout icon | sidebar -> `POST /api/v1/user/admin/logout` | Clears admin session and redirects login | Implemented + click guard |

## Test Coverage Mapping

| Layer | Coverage |
| --- | --- |
| Backend integration | Guard-route rejection, seeded doctors endpoint, patient register/login/logout flow, admin add-admin and message retrieval, appointment lifecycle (create/update/cancel/delete), doctor login appointment access |
| Frontend tests | Smoke render checks + register validation behavior + message submit behavior |
| Dashboard tests | Smoke render checks + admin login context behavior + add-doctor avatar validation behavior |
